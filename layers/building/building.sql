...
DROP VIEW IF EXISTS osm_all_buildings;
CREATE OR REPLACE VIEW osm_all_buildings AS (
         ...
         SELECT member AS osm_id, geometry, building AS classification,
         ...
         SELECT osm_id, geometry, building AS classification,
                  COALESCE(nullif(as_numeric(height),-1),nullif(as_numeric(buildingheight),-1)) as height,
                  COALESCE(nullif(as_numeric(min_height),-1),nullif(as_numeric(buildingmin_height),-1)) as min_height,
                  COALESCE(nullif(as_numeric(levels),-1),nullif(as_numeric(buildinglevels),-1)) as levels,
                  COALESCE(nullif(as_numeric(min_level),-1),nullif(as_numeric(buildingmin_level),-1)) as min_level,
         ...
         SELECT obp.osm_id, obp.geometry, obp.building AS classification,
...
DROP FUNCTION IF EXISTS layer_building(geometry, integer);
CREATE OR REPLACE FUNCTION layer_building(bbox geometry, zoom_level int)
RETURNS TABLE(geometry geometry, osm_id bigint, classification text, render_height int, render_min_height int, hide_3d boolean) AS $$
SELECT geometry,
       osm_id,
       classification,
       render_height,
...
FROM (
       ...
       UNION ALL
       -- etldoc: osm_building_polygon -> layer_building:z14_
       SELECT DISTINCT ON (osm_id) geometry,
                                   osm_id,
                                   classification,
